...
export default function TalentGreeter() {
  ...
  if (!greeter) {
    return (
      <div className="max-w-2xl">
        <div className="mb-2 text-[11px] uppercase tracking-widest text-gold font-bold">{t('greeter.eyebrow')}</div>
        <h1 className="font-serif text-3xl font-bold" style={{ color: 'var(--ds-t1)' }}>{t('greeter.matching')}</h1>
        <p className="mt-2 text-[14px]" style={{ color: 'var(--ds-t2)' }}>
          {t('greeter.matchingSub')}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl">
      <div className="mb-2 text-[11px] uppercase tracking-widest text-gold font-bold">{t('greeter.eyebrow')}</div>
      <h1 className="font-serif text-3xl font-bold" style={{ color: 'var(--ds-t1)' }}>{t('greeter.titleIn', { city: greeter.city })}</h1>
      <p className="mt-2 text-[14px]" style={{ color: 'var(--ds-t2)' }}>
        {t('greeter.sub')}
      </p>
      ...
      <div className="mt-8 rounded-2xl p-7" style={{ background: 'var(--ds-card)', border: '1px solid var(--ds-card-border)' }}>
        <div className="flex items-start gap-5">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-3 mt-2 text-[12.5px]" style={{ color: 'var(--ds-t2)' }}>
              ...
              <span>· {greeter.completed_missions} {t('greeter.missions')}</span>
            </div>
        ...
        <div className="mt-6 pt-6 grid sm:grid-cols-2 gap-3" style={{ borderTop: '1px solid var(--ds-card-border)' }}>
          <a href={`mailto:${greeter.email}`} className="rounded-xl p-4 flex items-center gap-3 transition" style={{ background: 'var(--ds-card-border)' }}>
            ...
            <div>
              <div className="text-[11px] uppercase tracking-wider" style={{ color: 'var(--ds-t3)' }}>{t('greeter.email')}</div>
        ...
        <div className="mt-6 pt-6" style={{ borderTop: '1px solid var(--ds-card-border)' }}>
          <div className="text-[11px] uppercase tracking-widest font-bold mb-3 inline-flex items-center gap-1.5" style={{ color: 'var(--ds-t2)' }}>
            <MessageSquare className="w-3 h-3" /> {t('greeter.conversation')}
          </div>
          <div className="rounded-xl p-4 max-h-72 overflow-y-auto space-y-3 bg-black/[0.05] dark:bg-white/[0.04]">
            {thread.length === 0 && (
              <div className="text-center text-[12px] py-4" style={{ color: 'var(--ds-t2)' }}>{t('greeter.noMessages')}</div>
            )}
            {thread.map((m) => {
              ...
              return (
                <div key={m.id} className={`max-w-[80%] ${mine ? 'ml-auto text-right' : ''}`}>
                  <div className="text-[10px] mb-0.5" style={{ color: 'var(--ds-t3)' }}>
                    {m.sender_name} · {relativeTime(m.timestamp, lang)}
                  </div>
                  <div
                  ...
                  {mine && m.read && <div className="text-[10px] mt-0.5 inline-flex items-center gap-1" style={{ color: 'var(--ds-t3)' }}><CheckCheck className="w-2.5 h-2.5" /> {t('greeter.read')}</div>}
                </div>
              );
            })}
            <div ref={endRef} />
          ...
          <div className="mt-3 flex gap-2">
            <textarea value={msg} onChange={(e) => setMsg(e.target.value)} rows={2} placeholder={t('greeter.placeholder')}
            ...
            <button onClick={onSend} disabled={!msg.trim()}
              ...
              <Send className="w-3.5 h-3.5" /> {t('common.send')}
            </button>
          </div>
        </div>
      </div>
