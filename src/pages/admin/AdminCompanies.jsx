    ...
    <div className="space-y-6 pb-12">
      ...
      {isLoading ? (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
          {filtered.map((c) => (
            <CompanyCard key={c.id} company={c} missions={missions} />
          ))}
        </div>
      )}
    </div>
...
function CompanyCard({ company: c, missions }) {
  ...
  return (
    <Card as={Link} to={`/admin/companies/${c.id}`} variant="default" interactive className="p-5 block">
      <div className="flex items-start gap-3 mb-3">
        ...
        <Pill tone="gold" size="xs">{TIER_LABEL[c.package_tier] || c.package_tier}</Pill>
      </div>
